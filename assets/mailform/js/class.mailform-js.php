			...
			if ( mailform_dt.eq(i).next( 'dd' ).find( 'input' ).length && mailform_dt.eq(i).next( 'dd' ).hasClass( 'required' ) ) {
				...
				if ( elements.eq(0).attr( 'type' ) === 'radio' || elements.eq(0).attr( 'type' ) === 'checkbox' ) {
					...
					if ( list_error === elements.length ) {
						...
						error++;
						scroll_point = compare_method( scroll_point, elements.eq(0).offset().top - header_height );
					} else {
...
EOM;
		...
		if ( $( 'input#mail_address_confirm' ).length && $( 'input#mail_address' ).length ) {
			...
			if ( element.val() !== '' && element.val() !== element_2.val() ) {
				...
				error++;
				scroll_point = compare_method( scroll_point, element.offset().top - header_height );
			} else {
...
EOM;
		...
		echo <<<EOM
			...
			if( $(window).width() >= 960 ) {
				$( 'html, body' ).animate({
					scrollTop: scroll_point - 21
				}, 500 );
			} else {
				$( 'html, body' ).animate({
					scrollTop: scroll_point - 59
				}, 500 );
			}
		}
